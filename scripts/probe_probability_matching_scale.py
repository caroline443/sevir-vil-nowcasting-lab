#!/usr/bin/env python3
"""Estimate a protocol-matched probability-matching loss weight."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="desired weighted PM gradient norm divided by MSE gradient norm",
    )
...
def main() -> int:
    ...
    model.eval()
    mse_fn = nn.MSELoss()
    pm_fn = ProbabilityMatchingLoss().to(device)
    ...
    for batch_index, batch in enumerate(loader):
        ...
        pm = pm_fn(prediction, targets)
        mse_grad = torch.autograd.grad(mse, prediction, retain_graph=True)[0]
        pm_grad = torch.autograd.grad(pm, prediction)[0]
        mse_norm = float(torch.linalg.vector_norm(mse_grad))
        pm_norm = float(torch.linalg.vector_norm(pm_grad))
        tiny = torch.finfo(torch.float32).tiny
        cosine = float(torch.sum(mse_grad * pm_grad)) / max(
            mse_norm * pm_norm, tiny
        )
        suggested = args.target_gradient_fraction * mse_norm / max(pm_norm, tiny)
        rows.append(
            {
                "batch": batch_index,
                "mse": float(mse),
                "probability_matching_loss": float(pm),
                "mse_gradient_norm": mse_norm,
                "probability_matching_gradient_norm": pm_norm,
    ...
    result = {
        "ok": True,
        "purpose": "probability_matching_gradient_scale_probe",
        "checkpoint": str(args.checkpoint),
        "amp_dtype": args.amp_dtype,
        "validation_batches": len(rows),
        "target_gradient_fraction": args.target_gradient_fraction,
        "published_weight_control": 10.0,
        "recommended_probability_matching_weight": statistics.median(
